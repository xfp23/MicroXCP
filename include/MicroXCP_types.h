...
typedef struct {
    MicroXcp_Entry_t entries[MICROXCP_ODTDATA_BYTE]; 
    uint8_t entry_count; // 这个 ODT 实际存了几个变量
    uint8_t pid; // odt的pid
} MicroXcp_Odt_t;

typedef struct {
    MicroXcp_Odt_t odts[MICROXCP_DAQODT_COUNT]; // 一个周期内可能要发 8 帧 CAN 报文
...
typedef struct  
{
    MicroXcp_DaqObj_t daq_list[MICROXCP_DAQLIST_COUNT];  
    MicroXcp_FindPid_t pid_list[6];

    uint32_t tick; 

    // uint8_t En; // 使能
    uint32_t entry_addr; // 变量地址
    uint32_t odt_addr; // dot地址
    uint32_t daq_addr; // daq列表地址
}MicroXcp_Context_t; 
