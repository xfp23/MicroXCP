...
void MicroXcp_ShortUploadResFunc()
{
    ...
    // XCP 协议要求 ShortUpload 后自动更新 MTA
...
void MicroXcp_GetDaqSizeResFunc()
{
    ...
    uint8_t data[8] = {0};
    data[0] = 0xFF;
    data[1] = MICROXCP_DAQODT_COUNT; // 当前 List 下包含的 ODT 数量
    data[2] = this->daq.daq_list[list_index].odts[0].pid; // 该 List 的第一个 PID
...
void MicroXcp_WriteDaqResFunc()
{
    ...
    // 修复协议偏移量，Byte 2 是 Size，Byte 4~7 是 Address
